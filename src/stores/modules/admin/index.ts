...
import { adminRouterList } from '@/router/modules/admin'
...
export const useAdminStore = defineStore('admin', () => {
   ...
   *  路由-博客模块首页路径
   */
  const ADMIN_HOME = import.meta.env.VITE_ROUTER_ADMIN_HOME_PATH
   ...
   *  菜单列表
   */
  const menuList = computed<RouterType.AdminRoute[]>(() => adminRouterList)

  /**
   *  搜索历史列表
   */
  const searchHistoryList = ref<RouterType.AdminRoute[]>([])

  /**
   *  已经打开的选项卡
   */
  const openedTabList = ref<RouterType.AdminRoute[]>([])

  /**
   *  当前激活的选项卡
   */
  const currentTab = ref<Partial<RouterType.AdminRoute>>({
  })
   ...
   * @param tab 选项卡对象
   */
  const addKeepAliveExclude = (tab: RouterType.AdminRoute) => {
   ...
   */
  const markTabsToRemove = (tabs: RouterType.AdminRoute[]) => {
    tabs.forEach((tab) => {
      if (tab.name) {
  ...
  const removeTab = (path: string) => {
    ...
    if (!openedTabList.value.length && path !== ADMIN_HOME) {
    ...
    if (currentTab.value.path === path) {
      if (currentTab.value.name) {
        addKeepAliveExclude(currentTab.value as RouterType.AdminRoute)
      }
  ...
  const removeOthers = (path: string) => {
    const tabsToRemove = openedTabList.value.filter(
      tab => tab.path !== path && tab.path !== ADMIN_HOME,
    )

    markTabsToRemove(tabsToRemove)
    openedTabList.value = openedTabList.value.filter(tab => tab.path === path || tab.path === ADMIN_HOME)
  }
  ...
  const removeAll = (path: string) => {
    if (path !== ADMIN_HOME) {
      ...
      const tabsToRemove = openedTabList.value.filter(tab => tab.path !== ADMIN_HOME)

      markTabsToRemove(tabsToRemove)
      openedTabList.value = openedTabList.value.filter(tab => tab.path === ADMIN_HOME)
  ...
  /**
   * 打开一个新的标签页或激活已存在的标签页
   * @param tab - 要打开的路由标签页对象，类型为AdminRouteRecordRaw
   */
  const openTab = (tab: RouterType.AdminRoute): void => {
  ...
  const checkFirstHomePage = () => {
    if (openedTabList.value.length && openedTabList.value[0].path !== ADMIN_HOME) {
      removeAll(ADMIN_HOME)
    }
  }
