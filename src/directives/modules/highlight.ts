 ...
 * 高亮代码
 * @param block - 代码块
 * @description 插入行号、添加复制按钮、分片处理代码块，解决大数据量一次写入卡顿问题 支持动态内容监听，确保所有代码块都能被正确处理
 */
 ...
 * 添加复制按钮
 * @param block - 代码块
 * @description 添加复制按钮，解决代码块复制按钮不显示问题
 */
function addCopyButton(block: HTMLElement) {
  ...
  copyButton.onclick = () => {
    // 过滤掉行号，只复制代码内容
    const codeContent = block.textContent?.replace(/^\d+\s+/gm, '') || ''

    navigator.clipboard.writeText(codeContent).then(() => {
      window.$notification.success('复制成功')
    })
  }
 ...
 * 检查代码块是否已经被处理过
 * @param block - 代码块
 * @description 检查代码块是否已经被处理过，解决代码块重复处理问题
 */
 ...
 * 标记代码块为已处理
 * @param block - 代码块
 * @description 标记代码块为已处理，解决代码块重复处理问题
 */
 ...
 * 查找并处理所有代码块
 * @param el - 元素
 * @description 查找并处理所有代码块，解决代码块重复处理问题
 */
 ...
 * @param delay - 延迟时间
 * @description 重试处理函数，解决代码块重复处理问题
 */
...
/**
 * 代码高亮、插入行号、复制按钮
 * @description 代码高亮、插入行号、复制按钮，解决代码块重复处理问题
 */
export const useHighlight: Directive<HTMLElement> = {
