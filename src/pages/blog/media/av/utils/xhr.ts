...
function searchPageCodeCheck(
  ...
  siteItem: OnlinePlayType.SiteItem_parser,
  CODE: string,
) {
...
async function baseFetcher({ siteItem, targetLink, CODE }: Args): Promise<OnlinePlayType.FetchResult> {
  ...
  try {
    ...
    if (siteItem.fetchType === 'get') {
      ...
      return {
        ...searchPageParser(response.responseText, siteItem, CODE),
      }
    }
  }
  catch {
...
/** jable 有些域名是带 -c */
async function javbleFetcher(args: Args): Promise<OnlinePlayType.FetchResult> {
