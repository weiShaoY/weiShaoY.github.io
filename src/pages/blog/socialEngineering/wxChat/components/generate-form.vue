...
<template>
  <el-card :header="title">
    <el-form :model="formState" label-width="auto" label-position="left">
      <!-- 文本 -->
      <template v-if="wxChatStore.activeChatType === 'text'">
        <el-input
          ref="textareaRef"
        ...
        <div class="max-h-[136px] flex flex-wrap overflow-y-auto bg-[#f9f9f9]">
          <Emoji @add="addEmoji" />
        </div>
      </template>

      <!-- 图片 -->
      <template v-else-if="wxChatStore.activeChatType === 'image'">
        <el-upload
          drag
          accept="image/*"
        ...
        >
          <div v-if="formState.image" class="">
            <img
          ...
          <div v-else class="">
            <div class="text-7 font-bold">+</div>

            <div class="el-upload__text">点击或拖动图片到此区域</div>
          </div>
        </el-upload>
      </template>

      <!-- 转账 -->
      <template v-else-if="wxChatStore.activeChatType === 'transferAccounts'">
      ...
      <!-- 红包 -->
      <template v-else-if="wxChatStore.activeChatType === 'redEnvelope'">
      ...
      <!-- 语音 -->
      <template v-else-if="wxChatStore.activeChatType === 'voice'">
        ...
        <el-form-item label="是否已读">
          <el-switch v-model="formState.voiceRead" />
        </el-form-item>
      </template>

      <!-- 音视频 -->
      <template v-else-if="wxChatStore.activeChatType === 'avInvite'">
        <el-form-item label="类型">
          <el-radio-group v-model="formState.avInviteType">
            <el-radio value="audio"> 音频邀请 </el-radio>

            <el-radio value="video"> 视频邀请 </el-radio>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="状态">
          <el-select v-model="formState.avInviteState" placeholder="请选择状态">
            <el-option label="通话完成" value="success" />
        ...
        <el-form-item v-if="formState.avInviteState === 'success'" label="时长">
          <div class="mb-2 w-full flex items-center gap-3">
            <el-select v-model="formState.avInviteHour" placeholder="小时">
              <el-option
                v-for="item in 24"
                :key="item - 1"
            ...
            <el-select v-model="formState.avInviteMinute" placeholder="分钟">
              <el-option
                v-for="item in 60"
                :key="item - 1"
                :label="String(item - 1).padStart(2, '0')"
            ...
            <el-select v-model="formState.avInviteSecond" placeholder="秒">
              <el-option
                v-for="item in 60"
                :key="item - 1"
                :label="String(item - 1).padStart(2, '0')"
      ...
      <!-- 名片 -->
      <template v-else-if="wxChatStore.activeChatType === 'businessCard'">
      ...
      <!-- 拍一拍 -->
      <template v-else-if="wxChatStore.activeChatType === 'takeAPat'">
        <el-form-item label="选择对象">
          <el-select v-model="formState.patRole" placeholder="请选择拍一拍对象">
        ...
        <el-form-item label="内容">
          ...
          <div class="mt-5">
            ...
            <span class="text-[18px] color-red font-bold">
              {{ patResult }}
            </span>
          </div>
        </el-form-item>
      ...
      <!-- 时间 -->
      <template v-else-if="wxChatStore.activeChatType === 'time'">
        <el-form-item label="选择时间">
          <div class="grid grid-cols-4 mb-2 w-full items-center gap-3">
            <el-select
            ...
            <el-select v-model="formState.datetime.hour" placeholder="小时">
              <el-option
                v-for="item in 24"
                :key="item - 1"
                :label="String(item - 1).padStart(2, '0')"
            ...
            <el-select v-model="formState.datetime.minute" placeholder="分钟">
              <el-option
                v-for="item in 60"
                :key="item - 1"
                :label="String(item - 1).padStart(2, '0')"
      ...
      <!-- 撤回 -->
      <template v-else-if="wxChatStore.activeChatType === 'revoke'">
        <el-button type="danger" @click="handleSentChat">
          发一条撤回信息
        </el-button>
      </template>

      <!-- 系统消息 -->
      <template v-else-if="wxChatStore.activeChatType === 'system'">
        <el-input
      ...
      <template
        ...
        "
      >
        <el-form-item label="消息被拒收" class="mt-2">
          <el-radio-group v-model="formState.rejected">
    ...
    <!-- 底部 -->
    <template v-if="!['revoke'].includes(wxChatStore.activeChatType)">
      <div class="my-5 w-full flex items-center justify-evenly">
        <el-button type="danger" plain @click="handleClearChat">
          {{ formType === "edit" ? "关闭" : "清空" }}
        </el-button>

        <el-button type="primary" plain @click="handleSentChat">
