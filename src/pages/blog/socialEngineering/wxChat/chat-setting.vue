...
watch(
  ...
  () => {
    const sendRole = !SYSTEM_TYPES.includes(wxChatStore.activeChatType as typeof SYSTEM_TYPES[number])
      ? wxChatStore.activeUserId === wxChatStore.DEFAULT_USER.id
        ? '你自己：'
        : `${wxChatStore.activeUser?.nickname}："''`
      : ''

    const type = addTypes.find(item => item.value === wxChatStore.activeChatType)
...
<template>
  ...
  >
    ...
    >
      <!-- 用户选择区域 -->
      <el-form-item
        label="编辑和选择用户（第一个用户默认是自己）"
      >
        <el-radio-group
          v-model="wxChatStore.activeUserId"
          class="w-full flex flex-wrap items-center rounded bg-gray-100 p-3"
        >
          <template
          ...
          >
            <div
              class="relative mx-3 h-22 w-22 flex flex-col cursor-pointer items-center justify-between rounded"
            >
              <!-- 头像区域 -->
              <div
              ...
              >
                <p
                  class="max-w-[80%] truncate text-sm"
                >
                  {{ user.nickname }}
                </p>
              </template>
      ...
      <!-- 消息类型选择 -->
      <el-tabs
        v-model="wxChatStore.activeChatType"
      >
        <el-tab-pane
