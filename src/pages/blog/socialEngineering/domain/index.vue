...
<template>
  <div class="h-full w-full flex flex-col gap-5 overflow-hidden">
    ...
    <a-spin :loading="isLoading">
      <a-descriptions :column="{ xs: 1, md: 1, lg: 3 }" bordered>
        <a-descriptions-item :span="2" label="单位名称">
          {{ domainData.icp.subject.name }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="单位性质">
          {{ domainData.icp.subject.nature }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="备案许可证编号">
          {{ domainData.icp.subject.license }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="信息更新时间">
          {{ domainData.icp.subject.updateTime }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="备案网站域名">
        ...
        <a-descriptions-item :span="1" label="域名状态">
        ...
        <a-descriptions-item :span="1" label="域名 DNS 服务器列表">
          <div class="flex flex-col items-start gap-2">
            <a-link
        ...
        <a-descriptions-item :span="1" label="域名创建时间">
          {{ handleDate(domainData.whois["Created Date"]) }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="域名更新时间">
          {{ handleDate(domainData.whois["Updated Date"]) }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="域名到期时间">
          {{ handleDate(domainData.whois["Expiry Date"]) }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="注册商名称">
          {{ domainData.whois.Registrar }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="DNS A 记录">
          <span v-for="item in domainData.dns.A"> {{ item }}<br /> </span>
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="DNS AAAA 记录">
          {{ domainData.dns.AAAA?.join(", ") }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="DNS CNAME 记录">
          {{ domainData.dns.CNAME?.join(", ") }}
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="DNS NS 记录">
          <span v-for="item in domainData.dns.NS"> {{ item }}<br /> </span>
        </a-descriptions-item>

        <a-descriptions-item :span="1" label="ISP和区域">
          {{
