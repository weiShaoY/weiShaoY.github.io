  ...
  try {
    ...
    if (type.value === 'qq') {
      const [res_phone, res_lol, res_oldPassword] = await Promise.all([
        AdminApi.getQqQueryPhone(searchValue.value),

        AdminApi.getQqQueryLol(searchValue.value),

        AdminApi.getQqQueryOldPassword(searchValue.value),
      ])
      ...
      const [res_qq, res_weiBo] = await Promise.all([
        AdminApi.getPhoneQueryQq(searchValue.value),

        AdminApi.getPhoneQueryWeiBo(searchValue.value),
      ])
      ...
      const res_qq = await AdminApi.getLolQueryQq(searchValue.value)
      ...
      const res_phone = await AdminApi.getWeiBoIdQueryPhone(searchValue.value)
