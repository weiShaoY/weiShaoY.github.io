...
async function getData() {
  try {
    ...
    if (category.value === 0) {
      const response = await AdminApi.getSexySentence()

      data.value.form = response.form
      data.value.content = response.content
    }
    else if (category.value === 1) {
      const response = await AdminApi.getJokeSentence()

      data.value.form = response.form
      data.value.content = response.content
    }
    else if (category.value === 2) {
      const response = await AdminApi.getLoveSentence()

      data.value.form = response.form
      data.value.content = response.content
    }
    else if (category.value === 3) {
      const response = await AdminApi.getOneSentence()

      data.value.form = response.form
      data.value.content = response.content
    }
    else if (category.value === 4) {
      const response = await AdminApi.getDogSentence()

      data.value.form = response.form
      data.value.content = response.content
    }
    else if (category.value === 5) {
      const response = await AdminApi.getEnglishSentence()
