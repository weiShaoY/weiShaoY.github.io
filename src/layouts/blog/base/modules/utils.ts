 ...
 * @param {RouterType.BlogRoute[]} routes - 路由列表
 * @returns {RouterType.BlogRoute | undefined} 匹配到的第一级路由项
 */
export function findTopRouteByPath(
  path: string,
  routes: RouterType.BlogRoute[],
): RouterType.BlogRoute | undefined {
  // 使用 Map 缓存已查找的路由，避免重复查找
  const routeCache = new Map<string, RouterType.BlogRoute>()
