...
async function actionSelect(value: any) {
  try {
    ...
    // 关闭左侧标签
      ...
      blogStore.freshTabList(copyTagList)
    ...
    // 关闭其他标签页
      ...
      blogStore.freshTabList(filterList)
      router.push({
        name: itemData.name,
      })
    }
    ...
    // 重新加载
      ...
      blogStore.deleteCache(itemData)
      ...
      blogStore.addCache(itemData.name)
    }

    // 关闭全部标签
    else {
      blogStore.resetTabList()
