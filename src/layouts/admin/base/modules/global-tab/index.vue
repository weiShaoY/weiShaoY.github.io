 ...
 * 计算属性
 */
const list = computed(() => adminStore.openedTabList) // 已打开的标签页列表
 ...
 * 点击标签页导航
 */
function clickTab(item: RouterType.AdminRoute) {
...
function closeWorkTab(type: string, tabPath: string) {
  ...
  switch (type) {
    case 'current':
      ...
      break
    case 'left':
      adminStore.removeLeft(path)
      break
    case 'right':
      adminStore.removeRight(path)
      break
    case 'other':
      adminStore.removeOthers(path)
      break
    case 'all':
      adminStore.removeAll(path)
      break
  }
  ...
  return [
    {
      key: 'left',
      label: '关闭左侧',
      icon: 'admin-tab-close-left',
      disabled: isFirstOrSecondTab,
    },
    {
      key: 'right',
      label: '关闭右侧',
      icon: 'admin-tab-close-right',
      disabled: isLastTab,
    },
    {
      key: 'other',
      label: '关闭其他',
      icon: 'admin-tab-close-other',
      disabled: isOneTab || disableOther,
    },
    {
      key: 'all',
      label: '关闭全部',
      icon: 'admin-tab-close-all',
      disabled: isOneTab,
    },
  ]
})
...
const topWidth = computed(() => {
  return `calc(100% - ${adminStore.setting.menu.leftMenuWidth + adminStore.setting.menu.rightMenuWidth}px`
})
</script>
...
<template>
  <div
    ...
    :style="{
      width: topWidth,
      height: `${adminStore.setting.tab.height}px`,
      top: `${adminStore.setting.header.height}px`,
    }"
  >
    <div
