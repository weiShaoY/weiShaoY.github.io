...
export function findTopRouteByPath(
  path: string,
  routes: RouterType.AdminRoute[],
): RouterType.AdminRoute | undefined {
  // 使用 Map 缓存已查找的路由，避免重复查找
  const routeCache = new Map<string, RouterType.AdminRoute>()
 ...
 */
export async function adminMenuJump(
  item: RouterType.AdminRoute,
