...
import { adminMittBus } from '@/utils'
...
/** 菜单列表 */
const menuList = computed(() => adminStore.menuList)
...
/** 搜索结果列表 */
const searchResult = ref<RouterType.AdminRoute[]>([])

/** 搜索历史记录列表的高亮索引 */
const historyHIndex = ref(0)

/** 搜索历史记录列表 */
const searchHistoryList = computed(() => adminStore.searchHistoryList)
...
function fuzzyQueryList(
  arr: RouterType.AdminRoute[],
  val: string,
): RouterType.AdminRoute[] {
  const lowerVal = val.toLowerCase()

  const result: RouterType.AdminRoute[] = []

  const searchItem = (item: RouterType.AdminRoute) => {
 ...
 * 跳转到搜索结果页面
 */
function searchGoPage(item: RouterType.AdminRoute) {
  adminMenuJump(item)
  addHistory(item)
  searchVal.value = ''
  searchResult.value = []
...
function updateHistory() {
  if (Array.isArray(searchHistoryList.value)) {
    adminStore.searchHistoryList = searchHistoryList.value
  }
}
 ...
 * 清理搜索项
 */
function cleanItem(item: RouterType.AdminRoute) {
 ...
 * 添加搜索历史
 */
function addHistory(item: RouterType.AdminRoute) {
...
onMounted(() => {
  adminMittBus.on('openSearchDialog', openSearchDialog)
  document.addEventListener('keydown', handleGlobalShortcut)
  document.addEventListener('keydown', handleKeyboardNavigation)
})

onUnmounted(() => {
  adminMittBus.off('openSearchDialog', openSearchDialog)
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <SvgIcon
              icon="admin-search-up"
              :size="24"
              color="#DCDFE8"
            />

            <SvgIcon
              icon="admin-search-down"
              :size="24"
              color="#DCDFE8"
            />
